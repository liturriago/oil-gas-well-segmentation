...
class SegmentationDataset(Dataset):
    def __init__(self, data_path: str, augmentation: bool = False, in_channels: int = 3):
        ...
        if self.dummy_mode:
            ...
            assert len(self.image_paths) == len(self.mask_paths), "Mismatched images and masks count"
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if cv2 is None or np is None:
            raise RuntimeError("cv2 and numpy are required. Install with `pip install opencv-python-headless numpy`")
